...
export class vfs {
	...
	static async readBinFolder () {
		if (isNode()) {
			return await fsp.readdir(vfs.getOutputPath());
		} else if (isBrowser()) {
			var dbName = vfs.getOutputPath().replace(':', '');
			var myDb = new PouchDB(dbName);
			myDb = myDb['بتش'];
			var docs = await myDb.allDocs();
			return docs.rows.map(row => row.id);
		}
	}
	static ئقرئمجلدخام = vfs.readBinFolder;
